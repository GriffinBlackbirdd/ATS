...
class ATSScorer:
    ...
    def _extract_section(self, text: str, keywords: List[str]) -> str:
        ...
        for line in text_lines:
            ...
            if capturing:
                ...
                is_section_header = line.strip().endswith(":") and any(
                    stop_word in line_lower
                    for stop_word in [
                        "education",
                        "experience",
                        "contact",
                ...
                if is_section_header and not any(
    ...
    def _analyze_contact_and_formatting(self, resume_text: str) -> Dict:
        ...
        contact_score = 0
        if contact_analysis["email"]:
            contact_score += 15
        if contact_analysis["phone"]:
            contact_score += 15
        if contact_analysis["linkedin"]:
            contact_score += 10
    ...
    def parse_job_requirements(self, jd_text: str) -> Dict:
        ...
        sections = {
            "required": self._extract_section(
                jd_text, ["requirements", "required", "must have"]
            ),
            "preferred": self._extract_section(
                jd_text, ["preferred", "nice to have", "bonus"]
            ),
            "responsibilities": self._extract_section(
                jd_text, ["responsibilities", "duties", "role"]
            ),
        }

        print(f"\nDEBUG - JD Analysis:")
        print(
            f"Required section length: {len(sections['required']) if sections['required'] else 0}"
        )
        print(
            f"Preferred section length: {len(sections['preferred']) if sections['preferred'] else 0}"
        )
        ...
        for category, skills_list in self.tech_skills.items():
            for skill in skills_list:
                ...
                if match:
                    ...
                    context = self._get_context(jd_lower, match)
                    print(
                        f"  Found skill '{skill}' directly in context: ...{context}..."
                    )

                elif skill in skill_expansion_map:
                    for related_term in skill_expansion_map[skill]:
                        related_pattern = re.compile(
                            r"\b" + re.escape(related_term.lower()) + r"\b"
                        )
                        related_match = related_pattern.search(jd_lower)
                        if related_match:
                            skill_found = True
                            context = self._get_context(jd_lower, related_match)
                            print(
                                f"  Found skill '{skill}' via related term '{related_term}' in context: ...{context}..."
                            )
                            break
                ...
                if skill_found:
                    ...
                    skill_in_required = sections["required"] and (
                        pattern.search(sections["required"].lower())
                        if match
                        else any(
                            re.compile(r"\b" + re.escape(term.lower()) + r"\b").search(
                                sections["required"].lower()
                            )
                            for term in skill_expansion_map.get(skill, [skill])
                        )
                    )
                    skill_in_preferred = sections["preferred"] and (
                        pattern.search(sections["preferred"].lower())
                        if match
                        else any(
                            re.compile(r"\b" + re.escape(term.lower()) + r"\b").search(
                                sections["preferred"].lower()
                            )
                            for term in skill_expansion_map.get(skill, [skill])
                        )
                    )
                    ...
                    if skill_in_required:
                        ...
                        if sections["responsibilities"] and (
                            pattern.search(sections["responsibilities"].lower())
                            if match
                            else any(
                                re.compile(
                                    r"\b" + re.escape(term.lower()) + r"\b"
                                ).search(sections["responsibilities"].lower())
                                for term in skill_expansion_map.get(skill, [skill])
                            )
                        ):
                            required_skills.append(skill)
                            print(
                                f"    → Found in responsibilities (marking as REQUIRED)"
                            )
                        else:
                            required_skills.append(skill)
                            print(f"    → Found in JD (defaulting to REQUIRED)")
    ...
    def analyze_resume(self, resume_text: str) -> Dict:
        ...
        for category, skills_list in self.tech_skills.items():
            for skill in skills_list:
                    ...
                    if skill.lower() == "r":
                        ...
                        for context_pattern in r_contexts:
                            if re.search(context_pattern, resume_lower, re.IGNORECASE):
                                found_r = True
                                print(
                                    f"  Found R programming language in appropriate context"
                                )
                                break
                ...
                if match:
                    ...
                    context = self._get_context(resume_lower, match)
                    print(
                        f"  Found resume skill '{skill}' directly in context: ...{context}..."
                    )
                    ...
                    for related_term in resume_skill_expansion[skill]:
                        ...
                        if related_match:
                            ...
                            context = self._get_context(resume_lower, related_match)
                            print(
                                f"  Found resume skill '{skill}' via related term '{related_term}' in context: ...{context}..."
                            )
                            break
        ...
        all_resume_skills = [
            skill for skills_list in found_skills.values() for skill in skills_list
        ]
        print(f"\nDEBUG - Resume Skills Found: {all_resume_skills}")
    ...
    def _calculate_skills_match(
        ...
        for req_skill in required_skills:
            ...
            if req_skill.lower() in resume_skills_lower:
                ...
                context_analysis = self._analyze_skill_context_in_jd(
                    req_skill, self._original_jd_text
                )
                ...
                if context_analysis["is_example_context"]:
                    user_cloud_skills = [
                        ...
                        if skill in cloud_equivalents
                    ]
                    if user_cloud_skills:
                        matched_skills.append(req_skill)
                        skill_matched = True
                        print(
                            f"  ✅ MATCHED (Cloud Equivalent): '{req_skill}' satisfied by user's cloud experience: {user_cloud_skills}"
                        )
                    else:
                        print(
                            f"  ❌ MISSING (Cloud Specific): '{req_skill}' mentioned as example but user has no cloud platforms"
                        )
                else:
                    print(
                        f"  ❌ MISSING (Cloud Specific): '{req_skill}' specifically required, not found in resume"
                    )

            elif req_skill.lower() in semantic_inference_map:
                specific_skills = semantic_inference_map[req_skill.lower()]
                found_specific = [
                    skill for skill in specific_skills if skill in resume_skills_lower
                ]
                ...
                if found_specific:
                    ...
                    skill_matched = True
                    print(
                        f"  ✅ MATCHED (Inferred): '{req_skill}' inferred from resume skills: {found_specific}"
                    )

            if not skill_matched:
                print(
                    f"  ❌ MISSING: '{req_skill}' NOT found in resume (neither directly nor through inference)"
                )

        match_percentage = (len(matched_skills) / len(required_skills)) * 100
        print(
            f"Skills match: {len(matched_skills)}/{len(required_skills)} = {match_percentage}%"
        )
        print(f"Final matched skills: {matched_skills}")
    ...
    def _calculate_keyword_coverage(
        ...
        print(
            f"Keyword coverage: {matches}/{len(jd_skills_set)} JD skills = {jd_coverage:.1f}%"
        )
    ...
    def calculate_ats_score(self, resume_analysis: Dict, jd_requirements: Dict) -> Dict:
        ...
        required_skills_pct = self._calculate_skills_match(
            resume_analysis["all_skills"], jd_requirements["required_skills"]
        )

        preferred_skills_pct = self._calculate_skills_match(
            resume_analysis["all_skills"], jd_requirements["preferred_skills"]
        )

        keyword_match_pct = self._calculate_keyword_coverage(
            resume_analysis["all_skills"], jd_requirements["all_skills"]
        )

        contact_formatting_pct = resume_analysis["contact_formatting"][
            "formatting_score"
        ]

        semantic_similarity_pct = 85

        required_skills_score = (required_skills_pct / 100) * self.weights[
            "required_skills"
        ]
        preferred_skills_score = (preferred_skills_pct / 100) * self.weights[
            "preferred_skills"
        ]
        keyword_score = (keyword_match_pct / 100) * self.weights["keyword_match"]
        contact_formatting_score = (contact_formatting_pct / 100) * self.weights[
            "contact_formatting"
        ]
        semantic_score = (semantic_similarity_pct / 100) * self.weights[
            "semantic_similarity"
        ]
        ...
        breakdown = self._generate_detailed_breakdown(
            ...
            jd_requirements,
            required_skills_pct,
            preferred_skills_pct,
            keyword_match_pct,
            contact_formatting_pct,
    ...
    def _generate_detailed_breakdown(
        ...
        for skill in jd_requirements["required_skills"]:
            ...
            if skill.lower() in resume_skills_lower:
                ...
                context_analysis = self._analyze_skill_context_in_jd(
                    skill, self._original_jd_text
                )

                if context_analysis["is_example_context"]:
                    user_cloud_skills = [
                        s for s in resume_skills_lower if s in cloud_equivalents
                    ]
                    if user_cloud_skills:
                        matched_required.append(skill)
                        skill_matched = True
                        print(
                            f"  ✅ Required skill MATCHED (Cloud Equivalent): {skill} satisfied by {user_cloud_skills}"
                        )
                    else:
                        print(
                            f"  ❌ Required skill MISSING (Cloud): {skill} - user has no cloud platforms"
                        )
                else:
                    print(
                        f"  ❌ Required skill MISSING (Cloud Specific): {skill} - specifically required"
                    )

            elif skill.lower() in semantic_inference_map:
                specific_skills = semantic_inference_map[skill.lower()]
                found_specific = [
                    s for s in specific_skills if s in resume_skills_lower
                ]
                ...
                if found_specific:
                    ...
                    skill_matched = True
                    print(
                        f"  ✅ Required skill MATCHED (Inferred): {skill} from {found_specific}"
                    )
        ...
        return {
            ...
            "recommendations": self._generate_recommendations(
                missing_required,
                missing_preferred,
                resume_analysis["contact_formatting"],
            ),
        }
    ...
    def _generate_recommendations(
        ...
        if missing_required:
            recommendations.append(
                f"Add missing required skills: {', '.join(missing_required[:3])}"
            )
        ...
        if contact_issues:
            recommendations.append(
                f"Add missing contact info: {', '.join(contact_issues)}"
            )
        ...
        if missing_sections:
            recommendations.append(
                f"Add missing essential sections: {', '.join(missing_sections)}"
            )

        # If no major issues, provide advanced recommendations
        if (
            not missing_required
            and not missing_preferred
            and not contact_issues
    ...
    def generate_report(self, resume_path: str, jd_path: str) -> Dict:
        ...
        return {
            ...
            "matching_details": {
                ...
                "keyword_match_rate": (
                    (
                        len(ats_results["breakdown"]["strong_matches"])
                        / len(jd_requirements["all_skills"])
                        * 100
                    )
                    if jd_requirements["all_skills"]
                    else 0
                ),
            },
        }
...
def main():
    ...
    try:
        ...
        report = ats_scorer.generate_report(
            "/Users/arreyanhamid/Developer/ai-resume/rendercv_output/Garv_Khurana_CV.pdf",
        ...
        print("-" * 100)
        print(
            f"📧 Email Address        : {'✅ Present' if contact_analysis['email'] else '❌ Missing'}"
        )
        print(
            f"📱 Phone Number        : {'✅ Present' if contact_analysis['phone'] else '❌ Missing'}"
        )
        print(
            f"🔗 LinkedIn Profile    : {'✅ Present' if contact_analysis['linkedin'] else '❌ Missing'}"
        )
        ...
        if jd_analysis:
            ...
            for i, skill in enumerate(jd_analysis.get("required_skills", []), 1):
                status = (
                    "✅ MATCHED"
                    if skill in report["detailed_analysis"]["strong_matches"]
                    else "❌ MISSING"
                )
        ...
        for category, skills in resume_skills.items():
            if skills:
                print(
                    f"🔧 {category.replace('_', ' ').title()} ({len(skills)} skills):"
                )
                for skill in skills:
                    match_status = (
                        "✅"
        ...
        print(f"✅ Skills Matched      : {skill_analysis['skills_matched']}")
        print(
            f"📈 Coverage Rate       : {(skill_analysis['skills_matched'] / skill_analysis['total_jd_skills'] * 100):.1f}%"
        )

        # Show unmatched resume skills
        jd_keywords = set(
            [skill.lower() for skill in jd_analysis.get("all_keywords", [])]
        )
        all_resume_skills = []
        for category_skills in report["resume_summary"]["skills_by_category"].values():
            all_resume_skills.extend(category_skills)
        ...
        unmatched_resume = [
            skill for skill in all_resume_skills if skill.lower() not in jd_keywords
        ]
